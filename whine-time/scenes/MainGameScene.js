...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "phaser", "../Grid", "../entities/Baby", "../entities/Human", "../entities/FeedingStation"], factory);
    }
})(function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.MainGameScene = void 0;
    const phaser_1 = __importDefault(require("phaser"));
    const Grid_1 = require("../Grid");
    const Baby_1 = require("../entities/Baby");
    const Human_1 = require("../entities/Human");
    const FeedingStation_1 = require("../entities/FeedingStation");
    class MainGameScene extends phaser_1.default.Scene {
        constructor() {
        ...
        create() {
            ...
            this.grid = new Grid_1.Grid(16, 12, 64);
        ...
        placeInitialEntities() {
            // Place initial baby
            const baby = new Baby_1.Baby('baby1', { x: 2, y: 2 }, 'Emma');
            this.placeEntity(baby);
            // Place initial human (female for breastfeeding)
            const human = new Human_1.Human('human1', { x: 4, y: 4 }, 'Sarah', 'female');
            this.placeEntity(human);
            // Place a feeding station
            const feedingStation = new FeedingStation_1.FeedingStation('fs1', { x: 6, y: 2 });
        ...
        getEntityFromItem(item) {
            ...
            switch (item.type) {
                case 'baby':
                    const baby = new Baby_1.Baby(item.id, item.position, item.data?.name || 'Baby');
                    baby.data = item.data;
                    return baby;
                case 'human':
                    const human = new Human_1.Human(item.id, item.position, item.data?.name || 'Human', item.data?.gender || 'female');
                    human.data = item.data;
                    return human;
                case 'feeding_station':
                    const fs = new FeedingStation_1.FeedingStation(item.id, item.position, item.data?.formulaType || 'regular');
                    fs.data = item.data;
                    return fs;
            }
            return null;
        ...
        handleEntityInteraction(entity) {
            if (entity instanceof Baby_1.Baby) {
                this.handleBabyInteraction(entity);
            }
            else if (entity instanceof Human_1.Human) {
                this.handleHumanInteraction(entity);
            }
            else if (entity instanceof FeedingStation_1.FeedingStation) {
